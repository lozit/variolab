...
> **v0.20.1 — no security delta vs v0.20.0.** Fixes the HubSpot form goal for HubSpot Forms V4: `tracker.js` now also listens for the V4 DOM event `hs-form-event:on-submission:success` (V4 does not broadcast the legacy `hsFormCallback` window message) and reads the form GUID via `HubSpotFormsV4.getFormFromEvent()` for the optional GUID match. Same conversion path (nonce + prior-impression + rate-limit + dedup); the DOM event is same-origin and only ever triggers the existing conversion call. No new SQL, REST endpoint, capability, input surface, or stored option.
>
> **v0.20.0 — no security delta vs v0.19.0.** Adds a "HubSpot form submitted" conversion goal. `tracker.js` adds a `window` `message` listener that fires a conversion only when: (1) `event.origin` is a HubSpot host (`hsforms.net`/`hsforms.com`/`hubspot.com` — browser-set, not spoofable cross-window), (2) `data.type === 'hsFormCallback'` with `eventName` `onFormSubmit(ted)`, and (3) an optional configured form GUID matches. The conversion path is unchanged: same REST endpoint with `wp_rest` nonce + prior-impression gate + per-IP rate limit + per-visitor dedup, so a stray/forged trigger is bounded (worst case: another HubSpot form on the same page counts — mitigated by the optional GUID). New goal type `Experiment::GOAL_HUBSPOT` is validated server-side; goal value is sanitized as before. No new SQL, REST endpoint, capability, or stored option. Preview-mode `console.log` of HubSpot messages runs only for logged-in `edit_posts` users.
>
> **v0.19.0 — no security delta vs v0.18.0.** Admin-only UI rework of the cache diagnostics on the A/B Tests list. Adds server-side detection chips (Kinsta / Cloudflare / cache plugin) read from `$_SERVER['HTTP_CF_RAY']`/`HTTP_CF_CONNECTING_IP` + Kinsta constants — boolean presence checks, no user input. Detected plugin name comes from a hardcoded allowlist (`detect_active_plugin()`) and is `esc_html()`-escaped; all box output uses `esc_html*`/`esc_attr*`/`esc_url`. Removes the top-of-page caching `admin_notices` handler (`CacheBypass::maybe_render_notice` + `has_running_experiment`) — pure output removal, folded into the list box. New localized JS flag `cacheDetectedServerSide` is a boolean. No new SQL, REST endpoint, input surface, capability, or stored option.
>
> **v0.18.0 — no security delta vs v0.17.1 (data-safety improvement).** Makes the uninstaller preserve all A/B data by default: `uninstall.php` now returns early unless `abtest_settings['delete_data_on_uninstall']` is explicitly truthy, so deleting the plugin no longer drops the events table or removes experiments/options. The new opt-in is a single checkbox saved via `! empty( $_POST['delete_data_on_uninstall'] )` (boolean coercion, no stored string), gated by the existing Settings nonce + `manage_options` capability path. No new SQL, REST endpoint, input surface, or capability; the destructive path is unchanged but now requires explicit consent — a net reduction in accidental-data-loss risk.
>
> **v0.17.1 — no security delta vs v0.17.0.** Admin-only UI polish on the cache-check: the explanatory note shows in the box only when a cache is detected (toggled client-side), and the resilient-mode pill reads "cache resilient mode". No server/data-flow change.
>
> **v0.17.0 — no security delta vs v0.16.0.** Adds an admin-only cache diagnostic (status pills on the experiments list). Probes run client-side from the admin browser (anonymous `fetch`) and send a custom request header `X-Abtest-Cache-Check`; the Router recognises it (`CacheBypass::is_cache_probe()`) to skip impression logging + variant cookie + cache-resilient redirect. The header only ever *suppresses* counting — fail-safe, no auth needed, and it does not change the cache key. New `CacheBypass::random_classic_url()` uses `get_posts()` (no user input). Results live in the admin's `localStorage`; no new REST endpoint, capability, SQL, or stored option beyond the existing `abtest_settings['cache_check_mode']`.
>
> **v0.16.0 — no security delta vs v0.15.11.** Adds an opt-in "cache-resilient mode" (default off): a static inline redirect script (`CacheBypass::cache_buster_script_tag()`, emitted via `wp_get_inline_script_tag()`) that bounces a cache-served test page to a unique `?_abtcb=…` URL. The script is hardcoded (no user input; the param value is `Date.now()`+`Math.random()` client-side), gated server-side to anonymous visitors on a matching experiment, and self-limiting (no redirect loop). New settings checkbox sanitised via `! empty()`; Cloudflare detection reads `HTTP_CF_RAY` only as a boolean presence check. No new input surface / capability / SQL / data-flow.
>
> **v0.15.11 — no security delta vs v0.15.10.** Adds two `add_filter()` exclusions (`rocket_delay_js_exclusions`, `perfmatters_delay_js_exclusions`) so delay-JS optimisers don't defer the conversion tracker until first interaction (the real "click twice" root cause — confirmed live: WP Rocket rewrote the tracker's `<script type>` to `rocketlazyloadscript`). The callback only appends two literal patterns to an array; no input surface / capability / data-flow.
>
> **v0.15.10 — no security regression vs v0.15.9 (slight improvement).** The conversion endpoint now derives the variant from the visitor's server-side impression instead of trusting the variant cookie (cookie used only when it matches an impression). This fixes a robustness bug (a missing/stale cookie lost the conversion — "click twice") and *strengthens* M1: a tampered cookie can no longer attribute a conversion to a different arm. Still requires a real impression (forgery rejected with `409 no_impression`), same rate-limit + dedup. Covered by `tests/Integration/ConvertControllerTest.php`.
>
