...
- _(nothing in flight — v0.20.1 released.)_
...
- [x] **v0.20.1 — fix HubSpot goal for Forms V4 (2026-06-15).** v0.20.0 only listened for the legacy `hsFormCallback` window message; diagnosed live on corporate.actirise.com that the embed is **HubSpot Forms V4** (`HubSpotFormsV4` global, `.hs-form-frame` data-form-id), which broadcasts **no** window message — it dispatches a DOM CustomEvent `hs-form-event:on-submission:success` instead (confirmed via HubSpot docs + the global's `getFormFromEvent`/`getForms` API; synthetic-dispatch plumbing test passed). `tracker.js` now listens for the V4 event (reads the GUID via `getFormFromEvent`) **and** keeps the legacy window-message path for older embeds, shared once-guard. PHPCS + unit + integration green. **Submit→conversion to confirm with one live test submit (preview console logs `[Variolab] HubSpot V4 submission success`).** Released (tag v0.20.1, wp.org).
- [x] **v0.20.0 — "HubSpot form submitted" conversion goal (2026-06-14).** Diagnosed live (corporate.actirise.com): the "Reserve my spot" button is a HubSpot form `<button>` inside a **cross-origin iframe** (`js-eu1.hsforms.net`) with a per-render dynamic `id` — so no CSS-selector click goal can ever reach it (browser boundary; clicks don't bubble to the parent, `querySelectorAll` can't see in). Added a third goal type `Experiment::GOAL_HUBSPOT`: `tracker.js` listens for HubSpot's `hsFormCallback`/`onFormSubmit(ted)` postMessage, verifies the HubSpot origin, optionally matches a form GUID (goal value), fires once (server dedup). Admin: new radio + help in `ExperimentEdit`; goal value optional for HubSpot (empty = any form); `validate`/`validate_multi` updated. Preview mode logs every HubSpot message to the console so one real submit confirms the wiring. PHPCS 32/32; 114 unit + 51 integration green; admin plumbing verified in wp-env (3 radios, value not required). **HubSpot submit→conversion to confirm with one live test submit (preview console).** Released (tag v0.20.0, wp.org).
