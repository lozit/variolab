...
final class Admin {
	...
	private const MENU_SLUG = 'abtest-experiments';
	...
	public function register_menu(): void {
		add_menu_page(
			__( 'A/B Testing', 'variolab-ab-testing' ),
			__( 'A/B Tests', 'variolab-ab-testing' ),
			'manage_options',
			self::MENU_SLUG,
			[ $this, 'render' ],
			'dashicons-chart-bar',
		...
		add_submenu_page(
			self::MENU_SLUG,
			__( 'Import HTML', 'variolab-ab-testing' ),
			__( 'Import HTML', 'variolab-ab-testing' ),
			'manage_options',
		...
		add_submenu_page(
			self::MENU_SLUG,
			__( 'Settings', 'variolab-ab-testing' ),
			__( 'Settings', 'variolab-ab-testing' ),
			'manage_options',
	...
	public function enqueue_assets( string $hook ): void {
		...
		if ( 'list' === $action ) {
			...
			// Cache-diagnostic pills: probe each running test URL (+ a baseline normal
			// page) anonymously from the browser to flag whether it's served from cache.
			$abtest_running_urls = \Abtest\CacheBypass::get_running_test_urls();
			if ( ! empty( $abtest_running_urls ) ) {
				wp_enqueue_script(
					'vlab-cache-check',
					ABTEST_PLUGIN_URL . 'assets/js/cache-check.js',
					[],
					ABTEST_VERSION,
					true
				);
	...
	public function render(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You are not allowed to access this page.', 'variolab-ab-testing' ), 403 );
		}
	...
	public function handle_save(): void {
		...
		if ( ! Experiment::is_transition_allowed( $baseline_for_transition, $status ) ) {
			...
			$transition_message = sprintf(
				...
				__( 'Status kept at %1$s — transition to %2$s is not allowed. Use the Resume action for paused experiments.', 'variolab-ab-testing' ),
		...
		if ( Experiment::STATUS_RUNNING === $effective_status ) {
			...
			if ( $conflict && (int) $conflict->ID !== $id ) {
				...
				$conflict_message = sprintf(
					...
					__( 'Saved as Draft because "%s" is already running on this URL. Pause that experiment first to start this one.', 'variolab-ab-testing' ),
		...
		// Trust model: scripts are admin-curated raw JS (Adwords pixels, FB pixels, Lemlist
		// beacons). The `unfiltered_html` capability is the WP-canonical gate for users who
		// can input raw JS (same gate used by the Custom HTML widget and the post editor's
		// "Custom HTML" block). Single-site admins have it by default; multisite admins
		// only get it explicitly granted by super-admins (correct: don't let a tenant
		// admin inject a pixel that fires on another tenant's traffic).
		// Position is sanitized via sanitize_key. The code body is stored verbatim — JS
		// regex / JSON payloads must survive the round-trip; sanitize_text_field would
		// strip newlines and tags. At OUTPUT time, UrlScripts::print_for_position() runs
		// each entry through wp_print_inline_script_tag() (the WP-blessed inline-script
		// helper), parsing any `<script ...>` wrapper the admin pasted to extract src /
		// async / defer / type attributes. Visitors only ever see code that went through
		...
		if ( '' !== $test_url && current_user_can( 'unfiltered_html' ) ) {
			$raw_scripts = isset( $_POST['url_scripts'] ) && is_array( $_POST['url_scripts'] )
				? wp_unslash( $_POST['url_scripts'] ) // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- raw code body is admin-trusted (unfiltered_html cap gate above); wrapped via wp_print_inline_script_tag at output
				: [];
			$entries     = [];
		...
		$this->redirect_with_notice( 'success', __( 'Experiment saved.', 'variolab-ab-testing' ), [ 'action' => 'edit', 'experiment' => $id ] );
	}
	...
	public function handle_status_change(): void {
		...
		if ( $id <= 0 || ! in_array( $status, $valid, true ) ) {
			$this->redirect_with_notice( 'error', __( 'Invalid status change.', 'variolab-ab-testing' ) );
		}
		...
		if ( ! Experiment::is_transition_allowed( $prev, $status ) ) {
			$this->redirect_with_notice(
				...
				sprintf(
					/* translators: 1: from status, 2: to status */
					__( 'Transition not allowed: %1$s → %2$s. Use the Resume action to re-run a paused experiment.', 'variolab-ab-testing' ),
					$prev,
					$status
				)
			);
			...
			if ( $conflict && (int) $conflict->ID !== $id ) {
				$this->redirect_with_notice(
					...
					sprintf(
						...
						__( 'Cannot start: "%s" is already running on this URL. Pause it first.', 'variolab-ab-testing' ),
						get_the_title( $conflict )
					)
				);
			}
		...
		$this->redirect_with_notice( 'success', __( 'Status updated.', 'variolab-ab-testing' ) );
	}
	...
	public function handle_resume(): void {
		...
		$id = isset( $_GET['experiment'] ) ? absint( wp_unslash( $_GET['experiment'] ) ) : 0;
		if ( $id <= 0 || ! get_post( $id ) ) {
			$this->redirect_with_notice( 'error', __( 'Invalid experiment.', 'variolab-ab-testing' ) );
		}
		...
		if ( Experiment::STATUS_PAUSED !== $current_status ) {
			$this->redirect_with_notice(
				'warning',
				__( 'Resume is only available for paused experiments.', 'variolab-ab-testing' )
			);
		}
		...
		if ( Experiment::STATUS_RUNNING === $status_after ) {
			$this->redirect_with_notice(
				...
				sprintf(
					...
					__( 'Resumed as a new experiment (#%d, now running). The original keeps its locked dates.', 'variolab-ab-testing' ),
					(int) $new_id
				)
			);
		}
		...
		$this->redirect_with_notice(
			...
			sprintf(
				/* translators: %d: new experiment ID */
				__( 'Created a new draft experiment (#%d) because another one is already running on this URL.', 'variolab-ab-testing' ),
				(int) $new_id
			)
		);
	}
	...
	public function handle_replace_running(): void {
		...
		$test_url = Experiment::get_test_url( $id );
		if ( '' === $test_url ) {
			$this->redirect_with_notice( 'error', __( 'This experiment has no test URL.', 'variolab-ab-testing' ) );
		}
		...
		if ( '' !== $paused_title ) {
			$this->redirect_with_notice(
				...
				sprintf(
					...
					__( 'Replaced "%1$s" (paused) with "%2$s" (now running).', 'variolab-ab-testing' ),
					$paused_title,
		...
		$this->redirect_with_notice(
			...
			sprintf(
				...
				__( '"%s" is now running.', 'variolab-ab-testing' ),
				(string) get_the_title( $id )
			)
		);
	}
	...
	public function handle_delete(): void {
		...
		$this->redirect_with_notice( 'success', __( 'Experiment deleted.', 'variolab-ab-testing' ) );
	}
	...
	private function validate_multi( string $title, string $test_url, array $variants, string $goal_type, string $goal_value, string $status, int $editing_id = 0 ): array {
		$errors = [];
		if ( '' === $title ) {
			$errors[] = __( 'Title is required.', 'variolab-ab-testing' );
		}
		if ( '' === $test_url ) {
			$errors[] = __( 'Test URL is required.', 'variolab-ab-testing' );
		} elseif ( ! self::is_valid_test_url( $test_url ) ) {
			$errors[] = __( 'Test URL must look like /path/ (lowercase, letters, numbers, hyphens, underscores).', 'variolab-ab-testing' );
		}

		if ( empty( $variants ) ) {
			$errors[] = __( 'Variant A is required.', 'variolab-ab-testing' );
		} else {
			$seen = [];
			foreach ( $variants as $i => $v ) {
				...
				if ( $pid <= 0 || ! get_post( $pid ) ) {
					$errors[] = sprintf(
						...
						__( 'Variant %s page does not exist.', 'variolab-ab-testing' ),
						Experiment::VARIANT_LABELS[ $i ] ?? '?'
					);
					continue;
				}
				if ( isset( $seen[ $pid ] ) ) {
					$errors[] = __( 'Each variant must use a different page — duplicates detected.', 'variolab-ab-testing' );
					break;
				}
				$seen[ $pid ] = true;
			}
		...
		if ( ! in_array( $status, $valid_status, true ) ) {
			$errors[] = __( 'Invalid status.', 'variolab-ab-testing' );
		}
		return $errors;
	}
	...
	private function validate( string $title, string $test_url, int $control_id, int $variant_id, string $goal_type, string $goal_value, string $status, int $editing_id = 0 ): array {
		...
		if ( $variant_id > 0 && ! get_post( $variant_id ) ) {
			$errors[] = __( 'Variant B page does not exist.', 'variolab-ab-testing' );
		}
