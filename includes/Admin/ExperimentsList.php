...
final class ExperimentsList {
	...
	public static function render(): void {
		...
		<div class="wrap abtest-wrap">
			<h1 class="wp-heading-inline"><?php esc_html_e( 'A/B Tests', 'variolab' ); ?></h1>
			<a href="<?php echo esc_url( $new_url ); ?>" class="page-title-action"><?php esc_html_e( 'Add new', 'variolab' ); ?></a>
			<a href="<?php echo esc_url( $csv_url ); ?>" class="page-title-action" title="<?php esc_attr_e( 'Download all visible experiments as CSV (respects current filters).', 'variolab' ); ?>">⬇ <?php esc_html_e( 'Export CSV', 'variolab' ); ?></a>

			<?php self::render_date_filter( $from, $to ); ?>

			<?php if ( empty( $experiments ) ) : ?>
				<p><?php esc_html_e( 'No experiments yet. Create your first A/B test.', 'variolab' ); ?></p>
				<?php return; ?>
			<?php endif; ?>
				...
				<p>
					...
					if ( ! $show_all && $hidden_count > 0 ) {
						printf(
							...
							esc_html__( 'No URLs with a running experiment. %s to see archived URLs.', 'variolab' ),
							'<a href="' . esc_url( add_query_arg( [ 'page' => Admin::menu_slug(), 'show' => 'all' ], admin_url( 'admin.php' ) ) ) . '">' . esc_html__( 'Show all', 'variolab' ) . '</a>' // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						);
					} else {
						esc_html_e( 'No experiments to display.', 'variolab' );
					}
					?>
				</p>
				<?php return; ?>
			...
			<?php foreach ( $grouped as $url => $exps ) : ?>
				...
				<div class="abtest-url-block">
					<h2 class="abtest-url-heading">
						...
						<?php else : ?>
							<code class="abtest-no-url"><?php esc_html_e( '(no URL set)', 'variolab' ); ?></code>
						<?php endif; ?>
						...
						<a href="<?php echo esc_url( $add_to_url ); ?>" class="page-title-action abtest-add-to-url">+ <?php esc_html_e( 'Add experiment to this URL', 'variolab' ); ?></a>
					</h2>

					<table class="wp-list-table widefat striped abtest-list">
						<thead>
							<tr>
								<th style="width:24%;"><?php esc_html_e( 'Experiment', 'variolab' ); ?></th>
								<th style="width:10%;"><?php esc_html_e( 'Status', 'variolab' ); ?></th>
								<th style="width:46%;"><?php esc_html_e( 'Variants', 'variolab' ); ?></th>
								<th style="width:8%;"><?php esc_html_e( 'Best', 'variolab' ); ?></th>
								<th style="width:12%;"><?php esc_html_e( 'Actions', 'variolab' ); ?></th>
							</tr>
						</thead>
						<tbody>
		...
		<tr>
					...
					<br><small class="abtest-muted">
						...
						if ( $started_at && $ended_at ) {
							printf(
								...
								esc_html__( '%1$s → %2$s (%3$s)', 'variolab' ),
							...
							printf(
								...
								esc_html__( 'Since %1$s (%2$s)', 'variolab' ),
								esc_html( mysql2date( get_option( 'date_format' ), $started_at ) ),
								esc_html( $duration )
							);
						}
			...
			<td>
				<?php if ( count( $labels ) <= 1 ) : ?>
					<span class="abtest-badge abtest-badge-baseline"><?php esc_html_e( 'Baseline', 'variolab' ); ?></span>
				<?php elseif ( null !== $multi['best'] ) : ?>
					<span class="abtest-badge abtest-badge-sig">
						<?php
						printf(
							/* translators: %s: variant label */
							esc_html__( '%s wins', 'variolab' ),
					...
					<span class="abtest-muted abtest-ci abtest-no-winner" title="<?php echo esc_attr( $reason ); ?>">
						...
						printf( esc_html__( 'No winner (α=%s)', 'variolab' ), esc_html( number_format_i18n( (float) $multi['alpha'], 3 ) ) );
						?>
			...
			<td class="abtest-actions">
				<a href="<?php echo esc_url( $edit_url ); ?>"><?php esc_html_e( 'Edit', 'variolab' ); ?></a>

				<?php if ( Experiment::STATUS_DRAFT === $status ) : ?>
					| <a href="<?php echo esc_url( self::status_url( $exp_id, Experiment::STATUS_RUNNING ) ); ?>"><?php esc_html_e( 'Start', 'variolab' ); ?></a>
				<?php elseif ( Experiment::STATUS_RUNNING === $status ) : ?>
					| <a href="<?php echo esc_url( self::status_url( $exp_id, Experiment::STATUS_PAUSED ) ); ?>"><?php esc_html_e( 'Pause', 'variolab' ); ?></a>
					| <a href="<?php echo esc_url( self::status_url( $exp_id, Experiment::STATUS_ENDED ) ); ?>"><?php esc_html_e( 'End', 'variolab' ); ?></a>
				<?php elseif ( Experiment::STATUS_PAUSED === $status ) : ?>
					| <a href="<?php echo esc_url( self::resume_url( $exp_id ) ); ?>"
						 class="abtest-resume"
						 title="<?php esc_attr_e( 'Create a new experiment from this one and start it. The original keeps its locked dates.', 'variolab' ); ?>"
						 onclick="return confirm('<?php echo esc_js( __( 'Resume by creating a new experiment with fresh dates? The original stays paused with its current period locked.', 'variolab' ) ); ?>');">
						<?php esc_html_e( 'Resume', 'variolab' ); ?>
					</a>
					| <a href="<?php echo esc_url( self::status_url( $exp_id, Experiment::STATUS_ENDED ) ); ?>"><?php esc_html_e( 'End', 'variolab' ); ?></a>
				<?php endif; ?>
				...
				<?php if ( Experiment::STATUS_RUNNING !== $status && Experiment::STATUS_ENDED !== $status && $running_other instanceof \WP_Post ) : ?>
					...
					<a href="<?php echo esc_url( self::replace_running_url( $exp_id ) ); ?>"
					   ...
					   title="<?php echo esc_attr( sprintf( /* translators: %s: title of the experiment that will be paused */ __( 'Pause "%s" and start this one in a single action.', 'variolab' ), get_the_title( $running_other ) ) ); ?>"
					   onclick="return confirm('<?php echo esc_js( sprintf( /* translators: 1: running experiment title, 2: new experiment title */ __( 'Replace "%1$s" (running) with "%2$s"? The current one will be paused.', 'variolab' ), get_the_title( $running_other ), get_the_title( $experiment ) ) ); ?>');">
						<?php esc_html_e( 'Replace running', 'variolab' ); ?>
					</a>
				<?php endif; ?>

				| <a href="<?php echo esc_url( self::delete_url( $exp_id ) ); ?>" class="abtest-delete" onclick="return confirm('<?php echo esc_js( __( 'Delete this experiment? Events will be lost.', 'variolab' ) ); ?>');"><?php esc_html_e( 'Delete', 'variolab' ); ?></a>
			</td>
		</tr>
		<?php
	}
	...
	private static function render_url_chart( string $url, array $exps, string $from = '', string $to = '' ): void {
		...
		<div class="abtest-chart-wrap">
			<h3 class="abtest-chart-title"><?php esc_html_e( 'Daily conversion rate', 'variolab' ); ?></h3>
	...
	private static function render_visibility_toggle( bool $show_all, int $hidden_count, string $from, string $to ): void {
		...
		<p class="abtest-visibility-toggle">
			<?php if ( $show_all ) : ?>
				<?php esc_html_e( 'Showing all URLs (including those without a running experiment).', 'variolab' ); ?>
				<a href="<?php echo esc_url( add_query_arg( $base, admin_url( 'admin.php' ) ) ); ?>"><?php esc_html_e( 'Show only running', 'variolab' ); ?></a>
			<?php elseif ( $hidden_count > 0 ) : ?>
				...
				?>
				<a href="<?php echo esc_url( add_query_arg( array_merge( $base, [ 'show' => 'all' ] ), admin_url( 'admin.php' ) ) ); ?>"><?php esc_html_e( 'Show all', 'variolab' ); ?></a>
			<?php endif; ?>
		</p>
		<?php
	}
				...
				<div class="abtest-variant-line">
					...
					<span class="abtest-variant-counts">
						...
						printf(
							...
							esc_html__( '%1$d / %2$d (%3$s)', 'variolab' ),
							(int) $row['conversions'],
							(int) $row['impressions'],
					...
					<?php if ( $label === $baseline ) : ?>
						<span class="abtest-variant-baseline"><?php esc_html_e( 'baseline', 'variolab' ); ?></span>
					<?php elseif ( $cmp ) : ?>
						...
						<?php if ( $cmp['significant'] ) : ?>
							<span class="abtest-badge abtest-badge-sig"><?php esc_html_e( 'sig', 'variolab' ); ?></span>
						<?php else : ?>
							<span class="abtest-muted abtest-ci">
								<?php
								printf(
									/* translators: 1: low bound, 2: high bound */
									esc_html__( '95%% CI [%1$s ; %2$s]', 'variolab' ),
	...
	private static function render_date_filter( string $from, string $to ): void {
		...
		<form method="get" class="abtest-date-filter">
			...
			<label>
				<?php esc_html_e( 'From', 'variolab' ); ?>
				<input type="date" name="from" value="<?php echo esc_attr( $from ); ?>" max="<?php echo esc_attr( $today ); ?>">
			</label>
			<label>
				<?php esc_html_e( 'To', 'variolab' ); ?>
				<input type="date" name="to" value="<?php echo esc_attr( $to ); ?>" max="<?php echo esc_attr( $today ); ?>">
			</label>
			<button type="submit" class="button button-secondary"><?php esc_html_e( 'Apply', 'variolab' ); ?></button>
			<span class="abtest-date-presets">
				<a href="<?php echo esc_url( $preset_url( [ 'from' => $preset_7, 'to' => $today ] ) ); ?>"><?php esc_html_e( 'Last 7 days', 'variolab' ); ?></a>
				·
				<a href="<?php echo esc_url( $preset_url( [ 'from' => $preset_30, 'to' => $today ] ) ); ?>"><?php esc_html_e( 'Last 30 days', 'variolab' ); ?></a>
				·
				<a href="<?php echo esc_url( $preset_url( [] ) ); ?>"><?php esc_html_e( 'All time', 'variolab' ); ?></a>
			</span>
			<?php if ( '' !== $from || '' !== $to ) : ?>
				<span class="abtest-date-active">
					...
					if ( '' !== $from && '' !== $to ) {
						...
						printf( esc_html__( 'Showing events from %1$s to %2$s', 'variolab' ), esc_html( $from ), esc_html( $to ) );
					} elseif ( '' !== $from ) {
						/* translators: %s: from date (YYYY-MM-DD) */
						printf( esc_html__( 'Showing events since %s', 'variolab' ), esc_html( $from ) );
					} else {
						/* translators: %s: to date (YYYY-MM-DD) */
						printf( esc_html__( 'Showing events up to %s', 'variolab' ), esc_html( $to ) );
					}
					?>
				</span>
			<?php endif; ?>
