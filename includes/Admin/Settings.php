...
final class Settings {
	...
	public static function render(): void {
		...
		<div class="wrap vlab-page abtest-wrap">
			...
			<form method="post" action="<?php echo esc_url( $action_url ); ?>" class="abtest-form">
				...
				<h2><?php esc_html_e( 'Privacy & consent (GDPR)', 'variolab-ab-testing' ); ?></h2>
				<p class="description">
					<?php
					printf(
				...
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'Require consent', 'variolab-ab-testing' ); ?></th>
						<td>
							<label>
								<input type="checkbox" name="require_consent" value="1" <?php checked( $req_consent ); ?>>
								<?php esc_html_e( 'Block tracking until the abtest_visitor_has_consent filter returns true', 'variolab-ab-testing' ); ?>
							</label>
							<p class="description">
				...
				<h2 class="abtest-section-title"><?php esc_html_e( 'Google Analytics 4', 'variolab-ab-testing' ); ?></h2>
				<p class="description">
					<?php esc_html_e( 'Forward every impression and conversion to GA4 via the Measurement Protocol. Fire-and-forget — never blocks page rendering.', 'variolab-ab-testing' ); ?>
				</p>

				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'Enable', 'variolab-ab-testing' ); ?></th>
						<td>
							<label>
								<input type="checkbox" name="ga4_enabled" value="1" <?php checked( ! empty( $ga4_cfg['enabled'] ) ); ?>>
								<?php esc_html_e( 'Send A/B events to GA4', 'variolab-ab-testing' ); ?>
							</label>
							<p class="description">
								<?php esc_html_e( 'Events sent: abtest_impression, abtest_conversion. Both include params: experiment_id, variant (A/B), test_url.', 'variolab-ab-testing' ); ?>
							</p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="abtest-ga4-id"><?php esc_html_e( 'Measurement ID', 'variolab-ab-testing' ); ?></label></th>
						<td>
							...
							<p class="description"><?php esc_html_e( 'Your GA4 Measurement ID (starts with "G-"). Find it under Admin → Data Streams in GA4.', 'variolab-ab-testing' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="abtest-ga4-secret"><?php esc_html_e( 'API Secret', 'variolab-ab-testing' ); ?></label></th>
						<td>
							...
							<p class="description">
								<?php esc_html_e( 'Generated in GA4 → Admin → Data Streams → your stream → Measurement Protocol API secrets.', 'variolab-ab-testing' ); ?>
							</p>
						</td>
					</tr>
				</table>

				<h2 class="abtest-section-title"><?php esc_html_e( 'Webhooks', 'variolab-ab-testing' ); ?></h2>
				<p class="description">
					<?php esc_html_e( 'POST every A/B event to one or more HTTP endpoints. Pipe to Zapier, Mixpanel, Segment, Slack, n8n, your data warehouse, anything that accepts JSON over HTTP. Fire-and-forget — never blocks page rendering.', 'variolab-ab-testing' ); ?>
				</p>

				<div class="abtest-webhooks" data-empty-msg="<?php esc_attr_e( 'No webhooks configured. Click + Add webhook.', 'variolab-ab-testing' ); ?>">
					<?php if ( empty( $webhooks ) ) : ?>
						<p class="abtest-webhooks-empty"><?php esc_html_e( 'No webhooks configured. Click + Add webhook.', 'variolab-ab-testing' ); ?></p>
					<?php else : ?>
	...
	private static function render_api_docs(): void {
		...
		?>
		<h2 class="abtest-section-title"><?php esc_html_e( 'REST API — pull stats', 'variolab-ab-testing' ); ?></h2>
		<p class="description">
			<?php esc_html_e( 'Pull experiment stats programmatically (n8n, Make, Pipedream, custom dashboards). Authenticated via WP Application Passwords.', 'variolab-ab-testing' ); ?>
		</p>
		<table class="form-table" role="presentation">
			<tr>
				<th scope="row"><?php esc_html_e( 'Endpoint', 'variolab-ab-testing' ); ?></th>
				<td>
					<input type="text" readonly class="large-text code" value="<?php echo esc_attr( $endpoint ); ?>" onclick="this.select();">
					<p class="description"><?php esc_html_e( 'Method: GET. Returns JSON with all matching experiments and their stats.', 'variolab-ab-testing' ); ?></p>
				</td>
			</tr>
			<tr>
				<th scope="row"><?php esc_html_e( 'Authentication', 'variolab-ab-testing' ); ?></th>
				<td>
					<p>
						<?php
						printf(
							/* translators: %s: link to user profile application passwords section */
							esc_html__( 'Generate a WP Application Password for your user (%s) — that user must have the manage_options capability. Use it in n8n as Basic Auth: username = your WP login, password = the generated 24-char string.', 'variolab-ab-testing' ),
							'<a href="' . esc_url( $apppw ) . '">' . esc_html__( 'your profile → Application Passwords', 'variolab-ab-testing' ) . '</a>' // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						);
						?>
					</p>
				</td>
			...
			<tr>
				<th scope="row"><?php esc_html_e( 'Query parameters', 'variolab-ab-testing' ); ?></th>
				<td>
					<ul class="abtest-api-params">
						<li><code>url=/promo/</code> — <?php esc_html_e( 'filter experiments to a single test URL', 'variolab-ab-testing' ); ?></li>
						<li><code>experiment_id=38</code> — <?php esc_html_e( 'fetch one experiment by ID', 'variolab-ab-testing' ); ?></li>
						<li><code>status=running</code> — <?php esc_html_e( 'filter by experiment status (draft, running, paused, ended)', 'variolab-ab-testing' ); ?></li>
						<li><code>from=YYYY-MM-DD&amp;to=YYYY-MM-DD</code> — <?php esc_html_e( 'restrict event date range for the stats computation', 'variolab-ab-testing' ); ?></li>
						<li><code>breakdown=daily</code> — <?php esc_html_e( 'include per-day series (for charting)', 'variolab-ab-testing' ); ?></li>
					</ul>
				</td>
			</tr>
			<tr>
				...
				<td>
					...
					?>
					<pre class="abtest-api-example"><?php echo esc_html( $curl_example ); ?></pre>
					<p class="description">
	...
	public static function render_webhook_row( int $index, array $hook ): void {
		...
		<div class="abtest-webhook-row">
			<div class="abtest-webhook-head">
				<label>
					<?php esc_html_e( 'Name:', 'variolab-ab-testing' ); ?>
					<input type="text" name="webhooks[<?php echo (int) $index; ?>][name]" value="<?php echo esc_attr( $hook['name'] ?? '' ); ?>" placeholder="<?php esc_attr_e( 'e.g. Slack alerts', 'variolab-ab-testing' ); ?>">
				</label>
				<label class="abtest-webhook-enabled">
					...
					<?php esc_html_e( 'Enabled', 'variolab-ab-testing' ); ?>
				</label>
				<a href="<?php echo esc_url( $test_url ); ?>" class="button button-secondary abtest-webhook-test"><?php esc_html_e( 'Send test event', 'variolab-ab-testing' ); ?></a>
				<button type="button" class="button-link abtest-webhook-remove" aria-label="<?php esc_attr_e( 'Remove this webhook', 'variolab-ab-testing' ); ?>">
					<?php esc_html_e( 'Remove', 'variolab-ab-testing' ); ?>
				</button>
			</div>
			<div class="abtest-webhook-fields">
				<label>
					<?php esc_html_e( 'URL', 'variolab-ab-testing' ); ?>
				...
				<label>
					...
					<select name="webhooks[<?php echo (int) $index; ?>][fire_on]">
						<option value="<?php echo esc_attr( Webhook::FIRE_ALL ); ?>" <?php selected( ( $hook['fire_on'] ?? Webhook::FIRE_ALL ), Webhook::FIRE_ALL ); ?>>
							<?php esc_html_e( 'All events (impressions + conversions)', 'variolab-ab-testing' ); ?>
						</option>
						<option value="<?php echo esc_attr( Webhook::FIRE_CONVERSION ); ?>" <?php selected( ( $hook['fire_on'] ?? Webhook::FIRE_ALL ), Webhook::FIRE_CONVERSION ); ?>>
							<?php esc_html_e( 'Conversions only (low volume)', 'variolab-ab-testing' ); ?>
						</option>
					</select>
				</label>
			</div>
	...
	public static function handle_save(): void {
		...
		if ( $enabled && '' !== $measurement_id && 0 !== strpos( $measurement_id, 'G-' ) ) {
			$warning = __( 'Measurement ID usually starts with "G-". Saved anyway — double-check it in GA4.', 'variolab-ab-testing' );
		}
		...
		$args = [
			...
			'action'          => 'settings',
			'abtest_notice'   => rawurlencode( '' !== $warning ? $warning : __( 'Settings saved.', 'variolab-ab-testing' ) ),
	...
	public static function handle_test_webhook(): void {
		...
		if ( ! isset( $webhooks[ $index ] ) ) {
			self::redirect_settings_notice( 'error', __( 'Webhook not found.', 'variolab-ab-testing' ) );
		}

		$hook = $webhooks[ $index ];
		if ( '' === $hook['url'] ) {
			self::redirect_settings_notice( 'error', __( 'This webhook has no URL configured.', 'variolab-ab-testing' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			self::redirect_settings_notice(
				...
				sprintf(
					...
					__( 'Test failed: %s', 'variolab-ab-testing' ),
		...
		$msg  = $code >= 200 && $code < 300
			? sprintf(
				...
				__( 'Test sent to "%1$s" — HTTP %2$d. Check the receiving end to confirm.', 'variolab-ab-testing' ),
				'' !== $hook['name'] ? $hook['name'] : $hook['url'],
				$code
			)
			: sprintf(
				/* translators: 1: webhook name or URL, 2: HTTP status code */
				__( 'Test sent to "%1$s" but endpoint replied HTTP %2$d. Check your endpoint config.', 'variolab-ab-testing' ),
				'' !== $hook['name'] ? $hook['name'] : $hook['url'],
				$code
			);
