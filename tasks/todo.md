...
- [x] **v0.18.0 — uninstall data safeguard + upgrade warning (2026-06-14).** Deleting the plugin used to run `uninstall.php`, which dropped the events table and removed every experiment, page-import record, and option — so users who "deleted + reinstalled" to upgrade silently lost all their A/B history (confirmed: this is exactly what bit the user across two sites). `uninstall.php` now returns early and **preserves everything by default**, only purging when `abtest_settings['delete_data_on_uninstall']` is explicitly truthy. New **Settings → Data & uninstall** section (`Settings.php`) explains "Update, don't Delete" and exposes an opt-in "Delete all data on uninstall" checkbox (off by default), saved via `! empty( $_POST['delete_data_on_uninstall'] )`; default added to `Plugin::activate()`'s `abtest_settings`. readme.txt: changelog + new FAQ "How do I upgrade without losing my experiments and stats?". Audit note (no security delta — destructive path now requires explicit consent). PHPCS clean; unit + integration green. **Built + verified locally; awaiting user go to release.**
- [x] **v0.17.1 — cache-check polish (2026-06-14).** (1) The explanatory "how to fix" note now lives in the Cache check box and is revealed by the JS only when an active cache is detected (baseline cached, or any test URL cached/handled) — `data-abtest-cache-note` + `updateNote()`. (2) The resilient-mode pill text changed from the ambiguous "cached (resilient)" to **"cache resilient mode"** (still amber). Verified locally with Playwright via injected localStorage state (note reveals on `baseOk`; handled pill renders "CACHE RESILIENT MODE" `--warn`; cached → red "CACHED"). **Built + verified; awaiting user go to release.**
- [x] **v0.17.0 — cache diagnostic pills on the A/B Tests list (2026-06-13).** Per-running-URL pill (out of cache ✓ / CACHED ⚠️ / cached-resilient) + a baseline pill for a random normal page (should be cached). Probes run client-side from the admin browser, anonymously (`credentials:'omit'`, so login cookies don't mask the cache), sending `X-Abtest-Cache-Check` → Router skips impression + cookie + cache-buster (`CacheBypass::is_cache_probe()`), so the check never pollutes stats (verified live: header-probe = 0 impressions, no-header anon load = 1). Results cached in `localStorage` (instant pills); refresh mode is a setting (`cache_check_mode`: smart/manual, default smart) so 50-test sites can avoid background bursts; probes batched (4). New `assets/js/cache-check.js`, `CacheBypass::random_classic_url()` + public `get_running_test_urls()`, `.vlab-cache-pill`/`.vlab-cache-bar` CSS. Tests: CacheProbeTest (unit), CacheBusterTest (+probe→no-redirect), CacheBypassClassicUrlTest. 114 unit + 51 integration green; PHPCS clean. Verified end-to-end in local wp-env with Playwright (pills render, modes, Re-check, stat-safety). Released (tag v0.17.0, wp.org).
- [x] **v0.16.0 — cache-resilient mode + Caching/CDN documentation (2026-06-13).** Live-diagnosed (headless browser on the user's Cloudways+Cloudflare site): the bare test URL is served from **Cloudways Varnish** (`Age: 274`, `x-cache: MISS` on fresh render; the plugin's `no-store` is stripped to `max-age=0, public`). On a cache hit WordPress never runs → no impression logged → conversion `409 no_impression` (and the 50/50 split is frozen). `CacheBypass` had no API to exclude Varnish/Cloudflare. Added: (1) **opt-in cache-resilient mode** (`abtest_settings['cache_resilient']`, default off) — `CacheBypass::cache_buster_script_tag()` injects a tiny redirect to a unique `?_abtcb=…` URL (high in `<head>` on themed via `wp_head` pri 0, and in blank-canvas's `<head>`), gated to anon visitors on a matching experiment, self-limiting; excluded from delay-JS. (2) **Docs**: new Settings → "Caching & CDN" section with host-specific exclusion steps (Cloudways/Varnish, Cloudflare, Kinsta) + prefix tip; admin notice now detects Cloudflare and links to both fixes; readme.txt FAQ entry. Tests: `CacheBusterTest` (4) + extended delay-JS unit test. 112 unit + 48 integration green; PHPCS clean. Released (tag v0.16.0, wp.org).
- [x] **v0.15.11 — exclude the tracker from delay-JS optimisers (the REAL "click twice" cause, 2026-06-12).** Diagnosed live with a headless browser on the user's `/test/` page: WP Rocket "Delay JavaScript Execution" had rewritten the tracker's `<script type>` to `rocketlazyloadscript` (and the inline config to `data-rocket-type`), so `window.AbtestTracker` stayed undefined until the first interaction — the first click only *woke* the script (after the click event), so the listener fired only from the second click. (The v0.15.10 cookie fix was real hardening but the wrong root cause.) Fix: `Tracker::register()` adds `rocket_delay_js_exclusions` + `perfmatters_delay_js_exclusions` filters excluding `variolab-ab-testing/assets/js/tracker.js` + `AbtestTracker`, so the tracker runs on load and the first click counts. +2 unit tests. 112 unit + 44 integration green. To re-verify post-deploy: load `/test/` in a browser, confirm `window.AbtestTracker` is defined *before* any interaction.
- [x] **v0.15.10 — conversions no longer depend on the variant cookie ("click twice" fix, 2026-06-12).** User reported having to click twice to record a conversion on an imported landing. Cause: `ConvertController` required `Cookie::get_variant()` (returned `400 no_variant_cookie` if absent), but the variant cookie can be missing on the first click — a CDN stripping `Set-Cookie`, or first-paint timing — while the impression is already logged server-side. Fix: new `Tracker::impression_variant()` derives the variant from the visitor's impression row; the controller prefers the cookie only when it matches an impression, else falls back to the impression. Cookie-independent (cache-resilient) and strictly more secure — a tampered cookie can no longer attribute a conversion to a different arm (the impression wins). New `tests/Integration/ConvertControllerTest.php` (3: logs-without-cookie, no-impression→409, tampered-cookie→impression-wins) + an `impression_variant` test. 110 unit + 44 integration green.
