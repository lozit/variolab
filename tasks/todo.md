...
- [x] **🚨 BLOCKER — Rename the plugin** ✅ DONE in v0.12.0 — see the "Trademark rename → "Uplift – A/B Testing"" item above for the full diff scope.
...
- [ ] **`PluginCheck.Security.DirectDB.UnescapedDBParameter` on `$table` interpolation** : false positive — `$table` always comes from `Schema::events_table()` (plugin-controlled). Already suppressed but reviewers sometimes ask anyway. If they do : extract the queries into a named helper that reads `$wpdb->prefix . 'abtest_events'` inline so the linter sees no variable.
